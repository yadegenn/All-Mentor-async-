...
GROUP_ID = -1002208163795
db_path = "OCDirect/bot.db"
WORK_CHAT_FILE = 'OCDirect/work_chat.txt'
    ...
    translators=[
        FluentTranslator(
            ...
            translator=FluentBundle.from_files(
                locale="ru-RU", filenames=["OCDirect/locales/ru.ftl"]
            )
        ),
    ],
    root_locale="ru"
...
async def handle_start(message, album: list = None, db=None, checker=None):
    ...
    if (is_photo_start):
        await bot.send_photo(message.chat.id, InputFile("OCDirect/main.png"), caption=welcome_message, parse_mode='HTML')
    else:
...
def setup_logging():
    ...
    error_log_dir = "OCDirect/logs/errors"
    debug_log_dir = "OCDirect/logs/debug"
