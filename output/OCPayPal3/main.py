...
# данные
TOKEN = '6712169314:AAFDPiRpYsBk9FBe5jVUuwHykhZpQw9HqRQ'
state_storage = StateMemoryStorage()
bot = AsyncTeleBot(TOKEN, state_storage=state_storage)
scheduler = AsyncIOScheduler()
GROUP_ID = -1002227671643
db_path = "OCPayPal3/bot.db"
WORK_CHAT_FILE = 'OCPayPal3/work_chat.txt'
    ...
    translators=[
        FluentTranslator(
            ...
            translator=FluentBundle.from_files(
                locale="ru-RU", filenames=["OCPayPal3/locales/ru.ftl"]
            )
        ),
    ],
    root_locale="ru"
...
async def handle_start(message, album: list = None, db=None, checker=None):
    ...
    if (is_photo_start):
        await bot.send_photo(message.chat.id, InputFile("OCPayPal3/main.png"), caption=welcome_message, parse_mode='HTML')
    else:
...
def setup_logging():
    ...
    error_log_dir = "OCPayPal3/logs/errors"
    debug_log_dir = "OCPayPal3/logs/debug"
