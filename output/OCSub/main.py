...
# данные
TOKEN = '6970295952:AAFxMXFiRH4sw1V1DeCG1PTCDHSkzfm0Or4'
state_storage = StateMemoryStorage()
bot = AsyncTeleBot(TOKEN, state_storage=state_storage)
scheduler = AsyncIOScheduler()
GROUP_ID = -1002150917285
db_path = "OCSub/bot.db"
WORK_CHAT_FILE = 'OCSub/work_chat.txt'
    ...
    translators=[
        FluentTranslator(
            ...
            translator=FluentBundle.from_files(
                locale="ru-RU", filenames=["OCSub/locales/ru.ftl"]
            )
        ),
    ],
    root_locale="ru"
...
async def handle_start(message, album: list = None, db=None, checker=None):
    ...
    if (is_photo_start):
        await bot.send_photo(message.chat.id, InputFile("OCSub/main.png"), caption=welcome_message, parse_mode='HTML')
    else:
...
def setup_logging():
    ...
    error_log_dir = "OCSub/logs/errors"
    debug_log_dir = "OCSub/logs/debug"
