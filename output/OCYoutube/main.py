...
GROUP_ID = -1002177452389
db_path = "OCYoutube/bot.db"
WORK_CHAT_FILE = 'OCYoutube/work_chat.txt'
    ...
    translators=[
        FluentTranslator(
            ...
            translator=FluentBundle.from_files(
                locale="ru-RU", filenames=["OCYoutube/locales/ru.ftl"]
            )
        ),
    ],
    root_locale="ru"
...
async def handle_start(message, album: list = None, db=None, checker=None):
    ...
    if (is_photo_start):
        await bot.send_photo(message.chat.id, InputFile("OCYoutube/main.png"), caption=welcome_message, parse_mode='HTML')
    else:
...
def setup_logging():
    ...
    error_log_dir = "OCYoutube/logs/errors"
    debug_log_dir = "OCYoutube/logs/debug"
