...
# данные
TOKEN = '6879915920:AAGhKkn3nVIQ-rdaorWMEAMmqyRysjdKYPo'
state_storage = StateMemoryStorage()
bot = AsyncTeleBot(TOKEN, state_storage=state_storage)
scheduler = AsyncIOScheduler()
GROUP_ID = -1002165037696
db_path = "PVTManager/bot.db"
WORK_CHAT_FILE = 'PVTManager/work_chat.txt'
DEVELOPER_ID = 5434361630
ADMINS = [5434361630, 629454540]
is_weekend_have = True
is_latehour_have = True
is_photo_start = False
weekend = False
latehour = False
send_weekend_users = []
send_latehour_users = []
    ...
    translators=[
        FluentTranslator(
            ...
            translator=FluentBundle.from_files(
                locale="ru-RU", filenames=["PVTManager/locales/ru.ftl"]
            )
        ),
    ],
    root_locale="ru"
...
async def handle_start(message, album: list = None, db=None, checker=None):
    ...
    if (is_photo_start):
        await bot.send_photo(message.chat.id, InputFile("PVTManager/main.png"), caption=welcome_message, parse_mode='HTML')
    else:
...
def setup_logging():
    ...
    error_log_dir = "PVTManager/logs/errors"
    debug_log_dir = "PVTManager/logs/debug"
