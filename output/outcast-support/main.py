...
GROUP_ID = -1002048973694
db_path = "outcast-support/bot.db"
WORK_CHAT_FILE = 'outcast-support/work_chat.txt'
    ...
    translators=[
        FluentTranslator(
            ...
            translator=FluentBundle.from_files(
                locale="ru-RU", filenames=["outcast-support/locales/ru.ftl"]
            )
        ),
    ],
    root_locale="ru"
...
async def handle_start(message, album: list = None, db=None, checker=None):
    ...
    if (is_photo_start):
        await bot.send_photo(message.chat.id, InputFile("outcast-support/main.png"), caption=welcome_message, parse_mode='HTML')
    else:
...
def setup_logging():
    ...
    error_log_dir = "outcast-support/logs/errors"
    debug_log_dir = "outcast-support/logs/debug"
