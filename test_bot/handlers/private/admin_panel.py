        ...
        if call.data == "get_data":
            async with pool.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    # Создание DataFrame
                    df = pd.DataFrame(rows, columns=['chat_id', 'topic_id', 'nickname', 'username', 'is_ban', 'reg_date'])
   ...
   if message.chat.id in ADMINS and message.document.mime_type == 'text/plain' and message.chat.type == 'private':
       try:
           ...
           df = await asyncio.to_thread(pd.read_csv, StringIO(data), delimiter='␟',
                                      names=['chat_id', 'topic_id', 'nickname', 'username', 'is_ban', 'reg_date'],engine='python')
           df = df.drop_duplicates(subset=['chat_id', 'topic_id'])

           async with pool.connection() as conn:
               async with conn.cursor() as cur:
                   await cur.execute('DELETE FROM users')
                   await cur.executemany('INSERT INTO users (chat_id, topic_id, nickname, username, is_ban, reg_date) VALUES (%s, %s, %s, %s, %s, %s)',
                                      df.values.tolist())
